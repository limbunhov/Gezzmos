    ...
    <div style="margin-bottom: 1rem;">
      <div>
        ...
        <div class="cart_items">
          ...
          <div class="d-flex justify-content-center m-auto loading" v-if="loadingproduct">
...
export default {
  ...
  methods: {
    ...
    async applySort() {
      this.loading = true;
      try {
        if (this.selectedSortOption.startsWith('model')) {
          // If sorting by model, sort alphabetically
          this.products = this.products.sort((a, b) => {
            const modelA = a.name.toLowerCase();
            const modelB = b.name.toLowerCase();

            if (this.selectedSortOption === 'model-asc') {
          ...
          // For other sorting options, use the existing logic
          await this.fetchProducts();
        }
      } catch (error) {
        console.error('Error fetching products:', error);
    ...
    async toggleFavorite(productId) {
      try {
        ...
        if (isFavorite) {
          ...
          const response = await axios.post('https://colorful-dog-teddy.cyclic.app/add/favorites', {
            userId: userId,
            productId: productId,
          });
          Swal.fire({
