...
export default {
  ...
  methods: {
    async fetchProduct() {
      try {
        const response = await axios.get(`https://colorful-dog-teddy.cyclic.app/products/${this.productId}`);
    ...
    async submitForm() {
      ...
      try {
        console.log(this.formData);
        const response = await axios.put(`https://colorful-dog-teddy.cyclic.app/products/${this.formData._id}`, this.formData);
